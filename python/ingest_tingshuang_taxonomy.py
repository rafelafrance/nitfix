...
def create_taxonomy_table(cxn, taxonomy):
    ...
    taxonomy.to_sql('taxonomy_ts', cxn, if_exists='replace', index=False)

    sql = """CREATE UNIQUE INDEX IF NOT EXISTS
             taxonomy_ts_sci_name ON taxonomy (sci_name)"""
    cxn.execute(sql)

    sql = """CREATE INDEX IF NOT EXISTS
             taxonomy_ts_genus ON taxonomy (genus)"""
    cxn.execute(sql)

    sql = """CREATE INDEX IF NOT EXISTS
             taxonomy_ts_family ON taxonomy (family)"""
    cxn.execute(sql)

    for i in range(1, 6):
        sql = f"""CREATE INDEX IF NOT EXISTS
                  taxonomy_ts_sample_id_{i} ON taxonomy (sample_id_{i})"""
        cxn.execute(sql)
...
def create_taxon_ids_table(cxn, taxonomy):
    ...
    taxonomy_ids.to_sql('taxonomy_ids_ts', cxn, if_exists='replace', index=False)

    sql = """CREATE INDEX IF NOT EXISTS
             taxon_ids_ts_sci_name ON taxonomy_ids (sci_name)"""
    cxn.execute(sql)

    sql = """CREATE INDEX IF NOT EXISTS
             taxon_ids_ts_sample_id ON taxonomy_ids (sample_id)"""
    cxn.execute(sql)
